...
static int mcrd_delete_ref_to_authkey(sc_card_t * card)
{
	...
	sc_format_apdu_ex(&apdu, 0x22, 0x41, 0xA4, 0, sbuf, 2, NULL, 0);
...
static int mcrd_delete_ref_to_signkey(sc_card_t * card)
{
	...
	if(card == NULL)
		return SC_ERROR_INTERNAL;
	sc_format_apdu_ex(&apdu, 0x22, 0x41, 0xB6, 0, sbuf, 2, NULL, 0);
	r = sc_transmit_apdu(card, &apdu);
	LOG_TEST_RET(card->ctx, r, "APDU transmit failed");
	SC_FUNC_RETURN(card->ctx, SC_LOG_DEBUG_VERBOSE, sc_check_sw(card, apdu.sw1, apdu.sw2));
}
				 ...
				 const sc_security_env_t * env, int se_num)
{
	...
	*p++ = 0;
	...
	case SC_SEC_OPERATION_DECIPHER:
	case SC_SEC_OPERATION_DERIVE:
		sc_format_apdu_ex(&apdu, 0x22, 0x41, 0xB8, 0, sbuf, 5, NULL, 0);
		break;
	case SC_SEC_OPERATION_SIGN:
		sc_format_apdu_ex(&apdu, 0x22, 0x41, 0xB6, 0, sbuf, 5, NULL, 0);
		break;
	default:
		return SC_ERROR_INVALID_ARGUMENTS;
	}
