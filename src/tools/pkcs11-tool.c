		...
		CK_OBJECT_HANDLE key)
{
	...
	if (r < (int) sizeof(in_buffer)) {
		...
		if (rv == CKR_USER_NOT_LOGGED_IN) {
			rv = p11->C_EncryptInit(session, &mech, key);
			if (rv != CKR_OK) {
				fprintf(stderr, "%s:%d C_EncryptInit failed with %s\n", __FILE__, __LINE__, CKR2Str(rv));
				p11_fatal("C_EncryptInit", rv);
			}
			login(session, CKU_CONTEXT_SPECIFIC);
			out_len = sizeof(out_buffer);
	...
	if (rv != CKR_OK) {
		...
		do {
			...
			if (rv == CKR_USER_NOT_LOGGED_IN) {
				rv = p11->C_EncryptInit(session, &mech, key); /* Hope, only 1st time */
				if (rv != CKR_OK) {
					fprintf(stderr, "%s:%d C_EncryptInit failed with %s\n", __FILE__, __LINE__, CKR2Str(rv));
					p11_fatal("C_EncryptInit", rv);
				}
			...
			if (rv != CKR_OK) {
				fprintf(stderr, "%s:%d C_EncryptUpdate failed with %s\n", __FILE__, __LINE__, CKR2Str(rv));
...
static int test_digest(CK_SESSION_HANDLE session)
{
	...
	unsigned char  *digests[] = {
        (unsigned char *) "\x7a\x08\xb0\x7e\x84\x64\x17\x03\xe5\xf2\xc8\x36\xaa\x59\xa1\x70", /* MD5 */
        (unsigned char *) "\x29\xb0\xe7\x87\x82\x71\x64\x5f\xff\xb7\xee\xc7\xdb\x4a\x74\x73\xa1\xc0\x0b\xc1", /* SHA_1 */
        (unsigned char *) "\xda\x79\xa5\x8f\xb8\x83\x3d\x61\xf6\x32\x16\x17\xe3\xfd\xf0\x56\x26\x5f\xb7\xcd", /* RIPEMD160 */
        (unsigned char *) "\x9c\xfe\x7f\xaf\xf7\x5\x42\x98\xca\x87\x55\x7e\x15\xa1\x2\x62\xde\x8d\x3e\xee\x77\x82\x74\x17\xfb\xdf\xea\x1c\x41\xb9\xec\x23", /* SHA256 */
        (unsigned char *) "\xa9\x8d\x73\x3c\xac\xb0\x6e\x11\xe\xd\xb5\x39\x28\x75\x13\x5\x8\x60\xed\xa5\x8\x1f\x2f\xfd\xd5\xab\xa3\xf3\x37\x37\x5\x18\x10\x4d\x10\x9d\x81\xf\x6a\x13\x9b\xb5\xea\x5f\x31\xba\xe3\xa2", /* SHA384 */
	...
	if (rv == CKR_OPERATION_NOT_INITIALIZED) {
		...
		if (rv == CKR_USER_NOT_LOGGED_IN) {
			...
			if (rv != CKR_OK)
				p11_fatal("C_DigestInit", rv);
			login(session, CKU_CONTEXT_SPECIFIC); // FIXME?
			rv = p11->C_Digest(session, data, sizeof(data), hash2, &hashLen2);
		}
		if (rv != CKR_OK) {
			fprintf(stderr, "%s:%d C_Digest failed with %s\n", __FILE__, __LINE__, CKR2Str(rv));
			p11_fatal("C_Sign", rv);
		}
	}
	...
	CK_OBJECT_HANDLE pub_key, int one_test)
{
	...
	for (j = 0, mech_type = mech_types; *mech_type != 0xffffff; mech_type++, j++) {
		...
		if (rv == CKR_MECHANISM_INVALID)
			continue;
		if (rv != CKR_OK) {
			printf(" %s:%d ERR: C_SignInit() returned %s (0x%0x)\n",
		...
		if (rv != CKR_OK) {
			printf(" %s:%d ERR: C_Sign() returned %s (0x%0x)\n",
				__FILE__, __LINE__, CKR2Str(rv), (int) rv);
			return ++errors;
		}
...
static void test_ec(CK_SLOT_ID slot, CK_SESSION_HANDLE session)
{
	...
	if (rv == CKR_USER_NOT_LOGGED_IN) {
		rv = p11->C_SignInit(session, &mech, priv_key);
		if (rv != CKR_OK) {
			fprintf(stderr, "%s:%d C_SignInit failed with %s\n", __FILE__, __LINE__, CKR2Str(rv));				p11_fatal("C_SignInit", rv);
		}
		login(session,CKU_CONTEXT_SPECIFIC);
		rv = p11->C_Sign(session, data, data_len, sig, &sig_len);
	}
